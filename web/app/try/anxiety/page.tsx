    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
      ...
      <div className="relative bg-[#111118] border border-cyan-500/20 rounded-2xl p-8 max-w-md w-full shadow-2xl shadow-cyan-500/5">
        ...
        {done ? (
          ...
          <>
            <h3 className="text-xl text-white mb-2" style={{ fontFamily: PF }}>Get your free calming frequency guide</h3>
            <p className="text-sm text-white/35 mb-6">Personalized protocols for nervous system regulation.</p>
...
export default function TryAnxietyPage() {
  ...
  const animateCountUp = useCallback((el: HTMLElement, target: string, gsap: any) => {
    ...
    const hasWks = target.includes('wks')
    const hasDays = target.includes('days')
    const num = parseInt(target.replace(/[^0-9]/g, ''), 10)
    if (hasWks || hasDays) {
      // Animate as-is with fade
  ...
  const scienceQuote = 'Five minutes of 528 Hz music significantly reduced salivary cortisol and increased oxytocin levels.'
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <section className="hero-section relative min-h-screen flex flex-col items-center justify-center px-6 text-center overflow-hidden">
        ...
        <div className="hero-glow absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-cyan-500/[0.06] blur-[120px] pointer-events-none" />
        <div className="relative max-w-2xl">
          <p className="hero-badge text-[11px] tracking-[0.3em] uppercase text-cyan-400/60 font-medium mb-8 opacity-0">432 Hz Natural Harmony</p>
          <h1 style={{ fontFamily: PF }}>
            <span className="hero-line-1 block text-4xl sm:text-5xl md:text-6xl font-light tracking-tight leading-[1.1] opacity-0">Quiet your nervous system.</span>
            <span className="hero-line-2 block text-4xl sm:text-5xl md:text-6xl font-light tracking-tight leading-[1.1] text-white/40 mt-2 opacity-0">In one session.</span>
          </h1>
          <p className="hero-sub text-base sm:text-lg text-white/35 max-w-md mx-auto mb-10 mt-8 leading-relaxed opacity-0">432 Hz activates your parasympathetic nervous system. Cortisol drops. Heart rate slows. No meditation experience needed.</p>
          <div className="hero-cta opacity-0">
            <Link href="/experience/2" className="cta-glow group inline-flex items-center gap-3 px-10 py-5 rounded-2xl bg-white text-gray-900 font-medium text-lg hover:bg-gray-100 transition-all shadow-2xl shadow-white/10">
              Try It Now — Free
      ...
      <section className="px-6 py-28 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-12">
          {[
            { n: '77%', d: 'of adults report daily physical symptoms caused by stress', target: '77%' },
            { n: '4-6 wks', d: 'before anxiety medication reaches full therapeutic effect', target: '4-6 wks' },
            { n: '95%', d: 'of meditation app users quit within 30 days', target: '95%' },
          ].map((s, i) => (
            <div key={i} className="border-t border-cyan-500/20 pt-6">
              <p className="stat-number text-5xl sm:text-6xl font-light text-cyan-400 mb-3" style={{ fontFamily: PF, fontVariantNumeric: 'tabular-nums' }} data-target={s.target}>{s.n === '4-6 wks' ? s.n : '0'}</p>
      ...
      <section className="px-6 py-28 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-0">
          {[
            { s: 'STEP 01', t: 'Headphones on', d: '432 Hz enters both ears. Your brain detects the frequency and begins synchronizing neural oscillations.' },
            { s: 'STEP 02', t: 'Entrainment begins', d: 'Neural oscillations shift from anxious beta to calm alpha within 3 to 5 minutes.' },
            { s: 'STEP 03', t: 'Calm arrives', d: 'Cortisol drops measurably. Heart rate variability improves. The anxiety loop breaks without effort.' },
          ].map((c, i) => (
            <div key={i} className={`step-col px-6 py-8 sm:py-0 opacity-0 ${i > 0 ? 'sm:border-l sm:border-l-white/[0.06]' : ''}`}>
              <div className="step-line h-[2px] bg-cyan-500/30 mb-6 origin-left" style={{ transform: 'scaleX(0)' }} />
      ...
      <section className="px-6 py-28 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6" style={{ perspective: '1000px' }}>
          {[
            { q: 'My therapist asked what changed. Resting heart rate dropped 8 bpm in three weeks.' },
            { q: 'I use it during panic episodes. Within 5 minutes, the chest tightness releases.' },
            { q: 'Replaced my evening wind-down routine entirely. 20 minutes of 432 Hz before bed.' },
          ].map((t, i) => (
      ...
      <section className="px-6 py-28">
        <div className="science-card max-w-3xl mx-auto bg-white/[0.02] border-l-2 border-cyan-500/40 rounded-2xl p-8 sm:p-12 relative overflow-hidden">
          ...
          <p className="text-sm text-white/30 mb-4">-- Akimoto et al., Health, 2018</p>
      ...
      <section className="px-6 py-28 max-w-3xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="space-y-10">
            {[
              { t: '0:00', h: 'Settle in. Close your eyes.', d: 'Find a comfortable position. Let your shoulders drop.' },
              { t: '2:00', h: 'Brain waves begin synchronizing.', d: 'Breathing naturally slows without you trying.' },
              { t: '5:00', h: 'Parasympathetic response activates.', d: 'Muscle tension releases. Mental chatter fades.' },
              { t: '10:00+', h: 'Full nervous system regulation.', d: 'The anxiety loop breaks. Clarity and calm replace the noise.' },
            ].map((item, i) => (
              <div key={i} className="timeline-item flex items-start gap-5">
                <div className="timeline-dot flex-shrink-0 w-2 h-2 rounded-full bg-cyan-400 mt-2" />
                <div>
                  <span className="text-xs font-mono text-cyan-400/70 block mb-1" style={{ fontVariantNumeric: 'tabular-nums' }}>{item.t}</span>
      ...
      <section className="px-6 py-28">
        <div className="final-cta max-w-3xl mx-auto bg-white/[0.02] border border-white/[0.06] rounded-2xl p-10 sm:p-14 text-center relative overflow-hidden">
          ...
          <div className="relative">
            <h2 className="text-2xl sm:text-4xl font-light mb-4" style={{ fontFamily: PF }}>Your nervous system already responds to 432 Hz.</h2>
            <p className="text-white/35 mb-8 text-sm sm:text-base">You have read the research. Now feel it.</p>
            <Link href="/experience/2" className="cta-glow group inline-flex items-center gap-3 px-10 py-5 rounded-2xl bg-white text-gray-900 font-medium text-lg hover:bg-gray-100 transition-all shadow-2xl shadow-white/10">
              Try It Now — Free
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:translate-x-1 transition-transform"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
            </Link>
      ...
      {showInline && (
        <section className="px-6 py-16 max-w-md mx-auto text-center">
          {inlineDone ? (
            ...
            <>
              <p className="text-white/40 text-sm mb-4">Not ready? Get the free calming guide.</p>
