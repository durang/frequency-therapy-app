...
function EmailCaptureModal({ show, onClose }: { show: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
      ...
      <div className="relative bg-[#111118] border border-indigo-500/20 rounded-2xl p-8 max-w-md w-full shadow-2xl shadow-indigo-500/5">
        ...
        {submitted ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="flex gap-3">
              ...
              <button type="submit" className="px-6 py-3 bg-indigo-500 text-white rounded-xl font-medium text-sm hover:bg-indigo-400 transition-colors whitespace-nowrap">Send</button>
            </form>
          </>
        )}
      </div>
  ...
  return (
    <div ref={pageRef} className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 text-center overflow-hidden">
        ...
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-indigo-500/[0.07] blur-[120px] pointer-events-none" />
        ...
        <div className="hero-content relative max-w-2xl">
          ...
          <p className="text-base sm:text-lg text-white/35 max-w-md mx-auto mb-10 leading-relaxed">
            1.5 Hz delta waves guide your brain into deep restorative sleep. No pills. No dependency. Just neuroscience.
          </p>
          <Link href="/experience/4"
      ...
      <section className="px-6 py-24 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {[
            { emoji: '😫', text: '50 million Americans have chronic sleep disorders' },
            { emoji: '💊', text: 'Sleep medication causes dependency and morning grogginess' },
            { emoji: '📱', text: 'Blue light from screens suppresses melatonin production by 50%' },
          ].map((card, i) => (
            <div key={i} className="pain-card bg-white/[0.02] border-l-2 border-indigo-500/30 rounded-xl p-6">
      ...
      <section className="px-6 py-24 max-w-4xl mx-auto">
        ...
        <div className="relative">
          <div className="hidden sm:block absolute left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-indigo-500/20 via-indigo-500/10 to-transparent" />
          <div className="space-y-12 sm:space-y-16">
            {[
              { num: '01', icon: '🎧', title: 'Put on headphones', desc: 'Delta frequencies at 1.5 Hz are delivered binaurally — below conscious hearing, directly to your brain' },
              { num: '02', icon: '🌊', title: 'Brain waves slow', desc: 'Neural activity gradually shifts from alert beta to deep delta over 15–20 minutes' },
              { num: '03', icon: '💤', title: 'Deep sleep activates', desc: 'Your body enters restorative sleep. Growth hormone releases. The brain\u2019s cleaning system activates.' },
            ].map((step, i) => (
              <div key={i} className="step-item flex items-start gap-6 sm:gap-8">
                <div className="flex-shrink-0 w-12 h-12 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center">
                  <span className="text-indigo-400 text-xs font-mono font-bold">{step.num}</span>
                </div>
                <div>
      ...
      <section className="px-6 py-24">
        <div className="science-card max-w-3xl mx-auto bg-white/[0.02] border border-indigo-500/15 rounded-2xl p-8 sm:p-12 relative overflow-hidden">
          <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full bg-indigo-500/[0.04] blur-[80px] pointer-events-none" />
          <div className="relative">
            <p className="text-lg sm:text-xl text-white/60 leading-relaxed italic mb-6" style={{ fontFamily: 'var(--font-playfair), Georgia, serif' }}>
              &ldquo;Music significantly reduces stress biomarkers and promotes autonomic relaxation in controlled studies.&rdquo;
            </p>
            <p className="text-sm text-white/30 mb-4">— Thoma et al., PLoS ONE, 2013</p>
            <a href="https://doi.org/10.1371/journal.pone.0070156" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-xs text-indigo-400/50 hover:text-indigo-400/80 transition-colors">
              <span className="w-4 h-4 rounded-full bg-indigo-500/20 flex items-center justify-center text-[10px]">✓</span>
              Published in PLoS ONE · View Study
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-24 max-w-3xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute left-[19px] top-2 bottom-2 w-px bg-gradient-to-b from-indigo-500/30 via-indigo-500/15 to-transparent" />
          <div className="space-y-10">
            {[
              { time: '0:00', title: 'Lights off. Headphones on.', desc: 'Get comfortable. Let your eyes close naturally.' },
              { time: '5:00', title: 'Beta waves fade. Alpha takes over.', desc: 'Your mind quiets. Thoughts slow down.' },
              { time: '12:00', title: 'Theta waves appear. You\u2019re drifting.', desc: 'The boundary between awake and asleep blurs.' },
              { time: '15:00+', title: 'Delta. Deep, restorative sleep.', desc: 'Your body repairs. Memories consolidate. You rest.' },
            ].map((item, i) => (
              <div key={i} className="timeline-item flex items-start gap-6">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center">
                  <span className="text-[10px] text-indigo-400 font-mono">{item.time}</span>
                </div>
                <div className="pt-1">
            ...
            <>
              ...
              <form onSubmit={handleInlineSubmit} className="flex gap-3">
                <input type="email" value={emailInline} onChange={e => setEmailInline(e.target.value)} placeholder="your@email.com" required
                  className="flex-1 px-4 py-3 bg-white/[0.05] border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:border-indigo-500/50 text-sm" />
                <button type="submit" className="px-6 py-3 bg-indigo-500 text-white rounded-xl font-medium text-sm hover:bg-indigo-400 transition-colors">Send</button>
              </form>
