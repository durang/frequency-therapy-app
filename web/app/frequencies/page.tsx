    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-[#fafaf9] dark:bg-[#0a0a0f] text-gray-900 dark:text-white transition-colors duration-500">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-10 text-center">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}
          className="text-4xl md:text-6xl font-light mb-4" style={{ fontFamily: 'var(--font-playfair), Georgia, serif' }}>
          Frequency Library
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 1 }}
          className="text-gray-500 dark:text-white/40 text-lg max-w-lg mx-auto" style={{ fontFamily: 'var(--font-instrument), Georgia, serif' }}>
      ...
      <div className="max-w-6xl mx-auto px-6 mb-8">
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.06] rounded-xl text-gray-900 dark:text-white/80 placeholder:text-gray-300 dark:placeholder:text-white/20 focus:outline-none focus:border-cyan-500/30 transition-all duration-300 text-sm"
          />
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        <div className={`grid gap-6 ${selectedFreq ? 'lg:grid-cols-[1fr,400px]' : ''}`}>
              ...
              return (
                ...
                >
                  <div className="relative z-10">
                    ...
                    <h3 className="text-lg font-light text-gray-900 dark:text-white/80 group-hover:text-gray-700 dark:group-hover:text-white mb-2"
                    ...
                    <div className="flex items-center gap-3 mt-3 text-[10px] text-gray-400 dark:text-white/20">
          ...
          {/* Detail panel — appears when a frequency is selected */}
          <AnimatePresence>
            {selectedFreq && (
              <motion.div
              ...
              >
                ...
                {selectedFreq.benefits?.length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="space-y-2">
                      {selectedFreq.benefits.slice(0, 4).map(b => (
                ...
                {selectedFreq.best_for?.length > 0 && (
                  <div className="mb-6">
                    <p className="text-xs text-gray-400 dark:text-white/25 uppercase tracking-wider mb-3">Best For</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedFreq.best_for.slice(0, 4).map(b => (
                        <span key={b} className="text-[10px] px-2.5 py-1 rounded-full border border-gray-200 dark:border-white/[0.06] text-gray-500 dark:text-white/30">{b}</span>
                      ))}
                    </div>
                  </div>
                )}
